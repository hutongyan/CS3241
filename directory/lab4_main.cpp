...
void DefineScene2(Scene &scene, int imageWidth, int imageHeight) {
    ...
    purple.k_d = Color(0.6f, 0.3f, 0.8f);
    purple.k_a = purple.k_d;
    purple.k_r = Color(0.8f, 0.8f, 0.8f) / 1.5f;
    purple.k_rg = Color(0.8f, 0.8f, 0.8f) / 3.0f;
    purple.n = 64.0f;

    // Light orange.
    Material lightOrange = Material();
    lightOrange.k_d = Color(0.9f, 0.5f, 0.2f);
    lightOrange.k_a = lightOrange.k_d;
    lightOrange.k_r = Color(0.8f, 0.8f, 0.8f) / 1.5f;
    lightOrange.k_rg = Color(0.8f, 0.8f, 0.8f) / 3.0f;
    lightOrange.n = 64.0f;

    // Insert into scene materials vector.
    scene.materials = { lightRed, lightGreen, lightBlue, yellow, gray, purple, lightOrange };
    ...
    auto horzPlane = new Plane(0.0, 1.0, 0.0, 0.0, scene.materials[2]); // Horizontal plane. vertical plane.
    auto leftVertPlane = new Plane( 1.0, 0.0, 0.0, 0.0, scene.materials[4] ); // Left vertical plane.
    ...
    Vector3d cubeCenter(30.0, 0.0, 50.0);

    for (int i = 0; i < 8; ++i) {
        double angle = i * M_PI / 4;  // 每45度放一个物体
        double radius = 15.0;         // 将半径增大到 15 以避免与立方体重合
        double x = 30.0+ radius * cos(angle);
        double z = 50.0 + radius * sin(angle);

        // 在立方体附近生成花瓣状的球体和三角形
        auto petalSphere = new Sphere(Vector3d(x, 5.0, z), 3.0, scene.materials[1]);
        scene.surfaces.push_back(petalSphere);

        auto connectingTriangle = new Triangle(cubeCenter + Vector3d(0.0, 5.0, 0.0), Vector3d(x, 5.0, z), Vector3d(x, 0.0, z), scene.materials[3]);
        scene.surfaces.push_back(connectingTriangle);
    }
