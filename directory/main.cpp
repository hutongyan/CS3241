...
void MakeReflectionImage( void )
{
    ...
    glLoadIdentity();
    glFrustum(TABLETOP_Y1 - eyePos[1], TABLETOP_Y2 - eyePos[1], TABLETOP_X1 - eyePos[0], TABLETOP_X2 - eyePos[0], eyePos[2] - TABLETOP_Z, eyeDistance + SCENE_RADIUS);
    
    // STEP 3: Sets up the imaginary viewpoint.
    float height = (2 * TABLETOP_Z) - eyePos[2];
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(eyePos[0], eyePos[1], height, eyePos[0], eyePos[1], eyePos[2], 1, 0, 0);
...
void DrawCube(void)
{         // Texture object for the cube
    GLfloat xTableLocation = 0.5f; // X-coordinate of the table location
    GLfloat yTableLocation = 0.5f; // Y-coordinate of the table location
    GLfloat tableHeight = 1.5f;    // Height of the table
    GLfloat cubeHeight = 0.8f;     // Height above the table for the floating cube

    // Define material properties for the floating cube
    GLfloat matAmbient[] = { 0.3, 0.3, 0.8, 1.0 };
    GLfloat matDiffuse[] = { 0.6, 0.6, 1.0, 1.0 };
    GLfloat matSpecular[] = { 0.8, 0.8, 1.0, 1.0 };
    ...
    // Move the cube above the tabletop
    glPushMatrix();
    glTranslated(xTableLocation, yTableLocation, tableHeight + cubeHeight);
    ...
    // Front face
    glNormal3f(0.0, 0.0, 1.0);
    glTexCoord2f(0.0, 0.0); glVertex3f(-0.3, -0.3,  0.3);
    glTexCoord2f(1.0, 0.0); glVertex3f( 0.3, -0.3,  0.3);
    glTexCoord2f(1.0, 1.0); glVertex3f( 0.3,  0.3,  0.3);
    glTexCoord2f(0.0, 1.0); glVertex3f(-0.3,  0.3,  0.3);
