...
void DrawDisc( const discType *d )
{
    ...
    if ( firstTime )
    {
        ...
        for (int i = 0; i < NUM_OF_SIDES + 1; i++) {
            double t = (double) i / NUM_OF_SIDES;
            unitDiscVertex[i][0] = cos(2 * PI * t);
            unitDiscVertex[i][1] = sin(2 * PI * t);
        }
    ...
    for (int i = 0; i < NUM_OF_SIDES + 1; i++) {
        glVertex2f(d->pos[0] + d->radius * unitDiscVertex[i][0], d->pos[1] + d->radius * unitDiscVertex[i][1]); // in counter-clockwise direction
    }
    glEnd();
}
...
void UpdateAllDiscPos( void )
{
    for ( int i = 0; i < numDiscs; i++ )
    {
        ...
        if (disc[i].pos[0] + disc[i].radius >= winWidth - 1) {
            disc[i].speed[0] *= -1;
        ...
        if (disc[i].pos[1] + disc[i].radius >= winHeight - 1) {
            disc[i].speed[1] *= -1;
...
int main( int argc, char** argv )
{
    ...
    MyTimer(0);//glutIdleFunc( UpdateAllDiscPos );  //*** MODIFY THIS ***
